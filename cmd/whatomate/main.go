...
func setupRoutes(g *fastglue.Fastglue, app *handlers.App, lo logf.Logger, basePath string, rdb *redis.Client, cfg *config.Config) {
	...
	g.GET("/api/accounts/{id}/phone-status", app.GetAccountPhoneStatus)
	g.POST("/api/accounts/{id}/request-code", app.RequestAccountPhoneVerificationCode)
	g.POST("/api/accounts/{id}/verify-code", app.VerifyAccountPhoneCode)
	g.POST("/api/accounts/{id}/register-phone", app.RegisterAccountPhone)
	g.POST("/api/accounts/{id}/two-step-verification", app.UpdateAccountTwoStepVerification)
	...
	g.DELETE("/api/contacts/{id}/messages/{message_id}", app.DeleteMessage)
	...
	g.GET("/api/rsvp-events/{id}/guest-candidates", app.ListRSVPGuestCandidates)
	g.GET("/api/rsvp-events/{id}/guests", app.ListRSVPGuests)
	g.POST("/api/rsvp-events/{id}/guests", app.AddRSVPGuests)
	g.POST("/api/rsvp-events/{id}/guests/import", app.ImportRSVPGuests)
	g.GET("/api/rsvp-events/{id}/reminders/preview", app.RSVPReminderPreview)
	g.POST("/api/rsvp-events/{id}/reminders/media", app.UploadRSVPReminderMedia)
	g.POST("/api/rsvp-events/{id}/reminders/send", app.SendRSVPReminders)
	...
	g.POST("/api/rsvp-events/{id}/reprompt", app.RepromptRSVPFlowSessions)
	g.POST("/api/rsvp-events/{id}/recover-partials", app.RecoverRSVPPartials)
