...
// Phone setup dialog state
const isPhoneSetupDialogOpen = ref(false);
const phoneSetupAccount = ref<WhatsAppAccount | null>(null);
...
function openPhoneSetupDialog(account: WhatsAppAccount) {
  phoneSetupAccount.value = account;
  isPhoneSetupDialogOpen.value = true;
}
...
const { isRefreshing: isRefreshingView, refreshNow } =
  useViewRefresh(fetchAccounts);
...
function openEditDialog(account: WhatsAppAccount) {
  ...
  formData.value = {
    ...
    marketing_messages_lite_onboarded:
      account.marketing_messages_lite_onboarded,
...
async function saveAccount() {
  if (
    ...
    !formData.value.phone_id.trim() ||
    !formData.value.business_id.trim()
  ) {
  ...
  try {
    ...
    if (editingAccount.value) {
      ...
      toast.success(
        t("common.updatedSuccess", { resource: t("resources.Account") }),
      );
    } else {
    ...
    toast.error(
      getErrorMessage(
        error,
        t("common.failedSave", { resource: t("resources.account") }),
      ),
    );
  } finally {
...
async function testConnection(account: WhatsAppAccount) {
  ...
  try {
    ...
    if (response.data.data.success) {
      ...
      toast.error(
        t("accounts.connectionFailed") +
          ": " +
          (response.data.data.error || "Unknown error"),
      );
    }
  } catch (error: any) {
          ...
          >
            <div class="p-6">
              <div class="flex items-start gap-4">
                <Skeleton
                  class="h-12 w-12 rounded-full bg-white/[0.08] light:bg-gray-200"
                />
                <div class="flex-1 space-y-3">
                  <Skeleton
                    class="h-5 w-48 bg-white/[0.08] light:bg-gray-200"
                  />
                  <div class="grid grid-cols-2 gap-2">
                    <Skeleton
                      class="h-4 w-32 bg-white/[0.08] light:bg-gray-200"
                    />
                    <Skeleton
                      class="h-4 w-32 bg-white/[0.08] light:bg-gray-200"
                    />
                  ...
                  <div class="flex gap-2">
                    <Skeleton
                      class="h-6 w-24 rounded-full bg-white/[0.08] light:bg-gray-200"
                    />
                    <Skeleton
                      class="h-6 w-24 rounded-full bg-white/[0.08] light:bg-gray-200"
                    />
      ...
      <div class="p-6">
        <div class="max-w-6xl mx-auto space-y-4">
          ...
          >
            <CardContent class="p-4">
              <div class="flex items-start gap-3">
                <AlertCircle
                  class="h-5 w-5 text-blue-400 light:text-blue-600 mt-0.5"
                />
                <div class="flex-1">
                  <h4 class="font-medium text-blue-100 light:text-blue-900">
                    {{ $t("accounts.webhookConfig") }}
                  </h4>
                  ...
                  <div class="flex items-center gap-2 mt-2">
                    <code
                      class="px-2 py-1 bg-blue-900 light:bg-blue-100 rounded text-sm font-mono"
                    >
                      {{ webhookUrl }}
                    </code>
                    <Button
          ...
          <div
            ...
            :key="account.id"
            class="account-card card-interactive rounded-xl border border-white/[0.08] bg-white/[0.02] light:bg-white light:border-gray-200"
          >
            <div class="p-6">
              <div class="flex items-start justify-between">
                <div class="flex items-start gap-4">
                  ...
                  <div class="min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                      <h3
                        class="font-semibold text-lg text-white light:text-gray-900"
                      >
                        {{ account.name }}
                      </h3>
                      <span
                    ...
                    <div v-if="testResults[account.id]" class="mt-2 space-y-2">
                      ...
                      >
                        ...
                        <span
                          v-if="testResults[account.id].display_phone_number"
                      ...
                      <div
                        v-else
                        class="flex items-center gap-2 text-red-400 light:text-red-600"
                      >
                        <X class="h-4 w-4" />
                        <span class="text-sm">{{
                      ...
                      <div
                        ...
                        class="flex items-start gap-2 p-2 rounded-lg bg-amber-950/50 light:bg-amber-50 border border-amber-800 light:border-amber-200"
                      >
                        <AlertCircle
                          class="h-4 w-4 text-amber-400 light:text-amber-600 mt-0.5 flex-shrink-0"
                        />
                        <span
                          class="text-sm text-amber-300 light:text-amber-700"
                    ...
                    <div class="mt-3 grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
                      ...
                      >
                        <span class="text-white/50 light:text-gray-500"
                          >App ID:</span
                        >
                        <code
                          class="text-xs bg-white/[0.08] light:bg-gray-100 px-1 rounded text-white/70 light:text-gray-600"
                          >{{ account.app_id }}</code
                        >
                      </div>
                      <div class="flex items-center gap-2">
                        ...
                        <code
                          class="text-xs bg-white/[0.08] light:bg-gray-100 px-1 rounded text-white/70 light:text-gray-600"
                          >{{ account.phone_id }}</code
                        >
                        <Button
                          variant="ghost"
                          size="icon"
                          class="h-6 w-6"
                          @click="copyToClipboard(account.phone_id, 'Phone ID')"
                        >
                          <Copy class="h-3 w-3" />
                        </Button>
                      </div>
                      ...
                      <div class="flex items-center gap-2">
                        <span class="text-white/50 light:text-gray-500"
                          >API Version:</span
                        >
                        <span class="text-white/70 light:text-gray-600">{{
                          account.api_version
                        }}</span>
                      </div>
                      <div class="flex items-center gap-2">
                        <span class="text-white/50 light:text-gray-500"
                          >{{ $t("accounts.accessToken") }}:</span
                        >
                        <Badge
                          variant="outline"
                          :class="
                    ...
                    <div class="mt-3 flex items-center gap-3 flex-wrap">
                      ...
                      <Badge
                        v-if="account.marketing_messages_lite_onboarded"
                        variant="outline"
                      >
                        <Check class="h-3 w-3 mr-1" />
                        MM Lite onboarded
                      ...
                      <Badge
                        v-if="account.marketing_messages_lite_enabled"
                        variant="outline"
                      >
                        <Check class="h-3 w-3 mr-1" />
                    ...
                    <div class="mt-3 flex items-center gap-2 text-sm">
                      <span class="text-white/50 light:text-gray-500"
                        >{{ $t("accounts.verifyToken") }}:</span
                      >
                      <code
                        class="text-xs bg-white/[0.08] light:bg-gray-100 px-2 py-0.5 rounded font-mono truncate max-w-[200px] text-white/70 light:text-gray-600"
                      >
                ...
                <div class="flex items-center gap-1">
                  ...
                  >
                    <Loader2
                      v-if="testingAccountId === account.id"
                  ...
                  <Tooltip>
                    <TooltipTrigger as-child>
                      ...
                      >
                        <Loader2
                          v-if="subscribingAccountId === account.id"
                          class="h-4 w-4 animate-spin"
                        />
                  ...
                  <Tooltip>
                    <TooltipTrigger as-child>
                      <Button
                        variant="ghost"
                        size="icon"
                        @click="openPhoneSetupDialog(account)"
                      >
                        <ShieldCheck class="h-4 w-4 text-emerald-500" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>{{
                      $t("accounts.phoneSetup")
                    }}</TooltipContent>
                  </Tooltip>
                  <Tooltip>
                    <TooltipTrigger as-child>
          ...
          >
            <div class="py-12 text-center text-white/50 light:text-gray-500">
              ...
              <p class="text-lg font-medium text-white light:text-gray-900">
          ...
          <Card>
            <CardContent class="p-6">
              ...
              <ol
                class="list-decimal list-inside space-y-3 text-sm text-muted-foreground"
              >
                <li>
                  {{ $t("accounts.setupStep1") }}
                  <a
                    href="https://developers.facebook.com"
                    target="_blank"
                    class="text-primary hover:underline inline-flex items-center gap-1"
                  >
                    {{ $t("accounts.metaDevConsole") }}
                    <ExternalLink class="h-3 w-3" />
                  </a>
                  {{ $t("accounts.setupStep1End") }}
                </li>
                <li>{{ $t("accounts.setupStep2") }}</li>
                <li>
                  {{ $t("accounts.setupStep3") }}
                  <strong>{{ $t("accounts.setupStep3Bold1") }}</strong>
                  {{ $t("accounts.setupStep3And") }}
                  <strong>{{ $t("accounts.setupStep3Bold2") }}</strong>
                </li>
                <li>
                  {{ $t("accounts.setupStep4") }}
                  <a
                    href="https://business.facebook.com/settings/system-users"
                    target="_blank"
                    class="text-primary hover:underline inline-flex items-center gap-1"
                  >
                    {{ $t("accounts.businessSettings") }}
                    <ExternalLink class="h-3 w-3" />
                  </a>
                </li>
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-2">
          <Label for="access_token">
            ...
            <span v-else class="text-muted-foreground">{{
              $t("accounts.accessTokenKeepExisting")
            }}</span>
          </Label>
          <Input
            id="access_token"
        ...
        <div class="space-y-2">
          <Label for="app_secret">
            ...
            <span v-if="editingAccount" class="text-muted-foreground">{{
              $t("accounts.accessTokenKeepExisting")
            }}</span>
          </Label>
          <Input
          ...
          <div class="rounded-lg border p-4 space-y-3">
            <div>
              ...
              <p class="text-xs text-muted-foreground mt-1">
                Use this only after Meta has approved and onboarded this
                WhatsApp Business Account for Marketing Messages Lite. It only
                applies to MARKETING templates.
              </p>
            </div>
            <div class="flex items-center justify-between gap-4">
              <Label
                for="marketing_messages_lite_onboarded"
              ...
              <Switch
                ...
                @update:checked="
                  formData.marketing_messages_lite_onboarded = $event;
                  if (!$event) formData.marketing_messages_lite_enabled = false;
                "
              />
            </div>
            <div class="flex items-center justify-between gap-4">
              <Label
                for="marketing_messages_lite_enabled"
                class="font-normal cursor-pointer"
              >
              ...
              <Switch
                ...
                @update:checked="
                  formData.marketing_messages_lite_enabled = $event
                "
              />
            </div>
            <p class="text-xs text-muted-foreground">
              Other message categories continue to use the standard Cloud API
              template flow.
            </p>
          </div>
        </div>
